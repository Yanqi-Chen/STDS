...
def train_one_epoch(model, criterion, optimizer, data_loader, device, epoch, print_freq, total_step, step, scaler=None):
    model.train()
    ...
    for image, target in metric_logger.log_every(data_loader, print_freq, header):
        ...
        if scaler is not None:
            with amp.autocast():
                output = model(image)
                print(f"Train Output: {output.device}")
        ...
        for module in model.modules():
            if hasattr(module, 'setFlatWidth'):
                if args.gradual == 'linear':
                    module.setFlatWidth(linearInc(step, total_step) * args.flat_width)
                elif args.gradual == 'sine':
                    module.setFlatWidth(sineInc(step, total_step) * args.flat_width)
...
def main(args):
    ...
    output_dir += f'_{args.sparse_function}'

    if ('flat' in args.sparse_function) or ('st' in args.sparse_function):
            output_dir += f'_flat_{args.flat_width}'
            
    if args.gradual:
        output_dir += f'_{args.gradual}'
    
    if args.alpha_gr != 0:
        output_dir += f'_alphagr{args.alpha_gr}'
    ...
    total_step = len(data_loader) * args.epochs
    print(f'train loader:{len(data_loader)}, #epoch:{args.epochs}')
    ...
    if args.adam:
        ...
        if args.alpha_gr == 0:
            ...
            for name, module in model.named_modules():
                if isinstance(module, sew_resnet.__dict__['PConv']) and module.weight.requires_grad:
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        train_loss, train_acc1, train_acc5 = train_one_epoch(model, criterion, optimizer, data_loader, device, epoch, args.print_freq, total_step, epoch * len(data_loader), scaler)
            ...
            if utils.is_main_process():
                ...
                for name, module in model.named_modules():
                    if hasattr(module, "getSparsity"):
                        ...
                        te_tb_writer.add_scalar(f'sparsity/{name}', zerocnt / numel, epoch)
                        if (epoch + 1) % 10 == 0 or epoch == 0:
                            te_tb_writer.add_histogram(f'w/{name}', module.getSparseWeight(), epoch)
                            te_tb_writer.add_histogram(f'theta/{name}', module.weight, epoch)
                print(f'total: {total_zerocnt / total_numel * 100:.2f}%')
                te_tb_writer.add_scalar(f'sparsity/total', total_zerocnt / total_numel, epoch)
